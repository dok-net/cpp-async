...
{
    // Like std::atomic, but defaults to std::memory_order_acq_rel (or memory_order_acquire or
    ...
    struct atomic_acq_rel final
    {
        ...
        {
            m_value.store(std::forward<T>(desired), std::memory_order_release);
        }
        ...
        {
            return std::forward<T>(m_value.load(std::memory_order_acquire));
        }
